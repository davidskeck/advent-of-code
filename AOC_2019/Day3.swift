...
struct Point: Hashable, Comparable {
    ...
    static func < (lhs: Point, rhs: Point) -> Bool {
        let lhsValue = abs(lhs.x) + abs(lhs.y)
        let rhsValue = abs(rhs.x) + abs(rhs.y)
...
func populateWireVectors(inputs: [Substring.SubSequence]) -> [WireVector] {
    var wireVectorResults: [WireVector] = []
    for input in inputs {
        let direction = Character(String(Array(input)[0]))
        let magnitudeIndex = input.index(input.startIndex, offsetBy: 1)
        let magnitudeRange = magnitudeIndex ..< input.endIndex
        if let magnitude = Int(input[magnitudeRange]) {
            if let direction = Direction(rawValue: direction) {
                wireVectorResults.append(WireVector(direction: direction, magnitude: magnitude))
            }
        }
    }
    return wireVectorResults
}

func traceWirePath(vectors: [WireVector]) -> WirePath {
    var wirePathResult = WirePath()
    for vector in vectors {
        switch vector.direction {
        case .up:
            // + y
            for _ in 1 ... vector.magnitude {
                wirePathResult.currentPosition.y += 1
                wirePathResult.points.append(wirePathResult.currentPosition)
            }
        case .down:
            // - y
            for _ in 1 ... vector.magnitude {
        ...
        case .left:
            // - x
            for _ in 1 ... vector.magnitude {
                wirePathResult.currentPosition.x -= 1
                wirePathResult.points.append(wirePathResult.currentPosition)
            }
        case .right:
            // + x
...
let wireInputs = puzzleInput.split { $0.isNewline }
let wireOneInputs = wireInputs[0].split { $0.isPunctuation }
let wireTwoInputs = wireInputs[1].split { $0.isPunctuation }

let wireOneVectors = populateWireVectors(inputs: wireOneInputs)
let wireTwoVectors = populateWireVectors(inputs: wireTwoInputs)

var wireOnePath = traceWirePath(vectors: wireOneVectors)
var wireTwoPath = traceWirePath(vectors: wireTwoVectors)

let wireOneSet = Set<Point>(wireOnePath.points)
let wireTwoSet = Set<Point>(wireTwoPath.points)

if let minimumPoint = (wireOneSet.intersection(wireTwoSet)).min() {
    let manhattanDistance = abs(minimumPoint.x) + abs(minimumPoint.y)
    print(manhattanDistance)
}

let intersectionSet = wireOneSet.intersection(wireTwoSet)

var intersectionDistances: [Int] = []
for intersection in intersectionSet {
    var wireOneDistance = 0
    var wireTwoDistance = 0
    for point in wireOnePath.points {
        wireOneDistance += 1
        if intersection == point {
            break
        }
    }
    for point in wireTwoPath.points {
        wireTwoDistance += 1
        if intersection == point {
            break
        }
    }
    intersectionDistances.append(wireOneDistance + wireTwoDistance)
}

if let minimumDistance = intersectionDistances.min() {
